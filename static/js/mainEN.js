...
// ==================== Dynamic Config & Status (Optimistic Skeleton) ====================
let APP_CONFIG = {
...
function bindGlobalEvents() {
    ...
    DOM.delegate('blur', '.description-input-dynamic', function () {
        if (!this.value.trim()) { this.classList.add('placeholder-active'); this.value = 'Needs cleaning or repair'; }
    });
...
function initChat() {
    ...
    setTimeout(() => {
        addBotMessage(`
            <div class="welcome-message">
                <div class="welcome-title">TAIPEI 101 Smart Service System</div>
                <div class="welcome-subtitle">Please select an area</div>
            </div>
        `);
        ...
        return `
            <div class="service-button" data-service="${key}">
                ...
                <div class="service-button-content">
                    ...
                    <div class="service-button-desc">${config.name_en} Report</div>
                </div>
            </div>
        `;
    }).join('');
...
function showReportForm() {
    ...
    const title = `📌 ${config.name_en} Form`;

    let locText = 'Select location';
    if (selectedLocation) locText = selectedLocation;
    else if (customLocation) locText = customLocation;

    let descVal = (problemDescription && problemDescription !== 'Needs cleaning or repair') ? problemDescription : 'Needs cleaning or repair';
    let descClass = 'form-control description-input-dynamic' + ((problemDescription && problemDescription !== 'Needs cleaning or repair') ? ' has-user-input' : ' placeholder-active');
...
function confirmForm() {
    ...
    if (descInput.classList.contains('placeholder-active') || !descVal || descVal === 'Needs cleaning or repair') {
        problemDescription = 'Needs cleaning or repair';
    } else {
...
// 🔥 修復第二次按「跳過」沒有動畫的 Bug：提前瞬間清場
function skipPhoto() {
    // 瞬間 (0秒) 刪除舊的確認表單與進度條，讓畫面回到乾淨狀態
    DOM.selectAll('.bot-message').forEach(m => {
        if (m.querySelector('.horizontal-case-card') || m.querySelector('.progress-indicator') || m.querySelector('.success-container')) m.remove();
    });
    ...
    if (uploadedFilePreview) {
        ...
        // 瞬間刪除舊的氣泡
        DOM.selectAll('.user-message').forEach(m => { if (m.textContent.includes('No photo upload') || m.textContent.includes('Photo added')) m.remove(); });

        // 滑順彈出新氣泡
        addUserMessage('No photo upload');
        // 等待 0.4 秒後，滑出最終表單
        setTimeout(() => setStep('confirmation'), 400);
    }
}

// 🔥 修復第二次按「確認」沒有動畫的 Bug：提前瞬間清場
function confirmPhoto() {
    ...
    // 瞬間刪除舊的氣泡
    DOM.selectAll('.user-message').forEach(m => { if (m.textContent.includes('Photo added') || m.textContent.includes('No photo upload')) m.remove(); });
...
function showConfirmation() {
    ...
    const html = `
        ...
        <div class="quick-replies confirmation-btn-group">
            <button class="quick-reply-btn edit-info-btn">✏️ Edit Info</button>
            <button class="quick-reply-btn submit-report-btn">✓ Submit</button>
        </div>`;
    addBotMessage(html);
}
    ...
    addBotMessage(`
        <div class="progress-indicator" id="${progressId}">
            ...
            <div class="progress-steps">
                <div class="progress-step active" data-step="1"><div class="step-icon">1</div><div class="step-label">Validate</div></div>
                <div class="progress-step" data-step="2"><div class="step-icon">2</div><div class="step-label">Process</div></div>
                <div class="progress-step" data-step="3"><div class="step-icon">3</div><div class="step-label">Send</div></div>
                <div class="progress-step" data-step="4"><div class="step-icon">4</div><div class="step-label">Done</div></div>
            </div>
...
function executeSubmitProcess(progressId) {
    ...
    if (uploadedFile) {
        ...
        photoPromise = compressAndUploadImage().then(id => {
            if (id) formData.photo_id = id; return formData;
        }).catch(() => formData);
    } else {
        updateProgress(2, 'Preparing data...');
        photoPromise = new Promise(resolve => setTimeout(() => resolve(formData), 400));
    }
...
async function showFloorModal() {
    if (!isConfigLoaded) {
        ...
        await Promise.all([
            configFetchPromise,
            new Promise(resolve => setTimeout(resolve, 1200)) // 保證至少停留 1.2 秒避免閃現
        ]);
        hideLoadingOverlay();
    }
    ...
    DOM.select('#floorModal').style.display = 'flex'; // 純原生顯示
}
...
async function showLocationModal() {
    ...
    DOM.select('#locationModal').style.display = 'flex'; // 純原生顯示
}
