...
// ==================== App Initialization (0s Optimistic Render + Background Fetch) ====================
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (autoDetectedLocation) {
                selectedLocation = autoDetectedLocation;
                customLocation = '';
            }
            setStep('input_form');
        }, 800);
...
// ==================== Modals & UI Controls (Anti-Ghost Click & Await API) ====================
...
async function showFloorModal() {
    ...
    if (!isConfigLoaded) {
        showToast("Loading latest floor data...", "info");
...
async function showLocationModal() {
    // 防護網：極限手速或網路過慢時，強制等待最新資料
    if (!isConfigLoaded) {
        showToast("Loading latest location data...", "info");
        await configFetchPromise;
    }
    if (!isConfigLoaded) return showToast("Failed to load data, please try again", "warning");
