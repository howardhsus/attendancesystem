...
// ==================== Cross-platform Modal Manager ====================
const ModalManager = {
    // Track currently open modal
    currentModal: null,

    // Open modal - Cross-platform fix version
    openModal($modal) {
    ...
    // Close modal - Cross-platform fix version
    closeModal($modal) {
        ...
        setTimeout(() => {
            ...
            // Restore body state
            if (!this.currentModal || this.currentModal[0] === $modal[0]) {
    ...
    bindCloseEvents($modal) {
        ...
        // Close on ESC key
        $(document).off('keyup.modal').on('keyup.modal', function (event) {
...
function closeModal($modal) {
    $modal.hide();
    // Fix issue 3: Restore background scrolling
    restoreBackgroundScroll();
}

// New: Prevent background scrolling
function preventBackgroundScroll() {
    // Record current scroll position - use standard scrollY
