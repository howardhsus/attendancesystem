...
// ==================== Cross-platform Button State Manager ====================
const ButtonStateManager = {
    // Track all button groups
    buttonGroups: {},
    ...
    // Handle button click - Cross-platform fix version
    handleButtonClick(groupId, $button) {
        ...
        if (group.singleSelect) {
            // Single select mode: clear all, then select current
            $(group.selector).removeClass('selected active');

            // Cross-platform fix: ensure button state is correctly set
            $button.addClass('selected');
            group.selected = buttonId;

            // Immediately remove possible :active state
            $button.blur();
        } else {
            // Multi-select mode: toggle state
    ...
    // Programmatically select button
...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function (e) {
        ...
        // 1. Clear all same type button states
        $('.floor-option-btn').removeClass('selected active');

        // 2. Set current button state - Cross-platform fix
        $btn.addClass('selected');

        // 3. Brief active state (visual feedback)
        $btn.addClass('active');
        setTimeout(() => {
...
function skipPhoto() {
    ...
    // Fix issue 2: Clear default selection state
...
// ==================== Modal Related Functions - Fix iOS Background Flicker Issue ====================

function showFloorModal() {
    // Fix issue 1: iOS background flicker
    ...
    // Fix issue 3: Clear previously stored state, only keep current selection
    const floorOptionsHTML = floors.map(floor => {
        const displayFloor = `${floor}F`;
        // Only show current selection state
...
function selectFloor(floor) {
    ...
    // Fix issue 3: First clear all button states
    $('.floor-option-btn').removeClass('selected active');
    ...
    // Use button state manager to ensure consistent state
...
function confirmCustomLocation() {
    ...
    // Fix issue 3: Clear previous location button states
    $('.location-option-btn').removeClass('selected active');
...
// New: Unified modal close function
function closeModal($modal) {
    $modal.hide();
    // Fix issue 1: Restore background scrolling
    restoreBackgroundScroll();
}

// New: Prevent background scrolling - Fix iOS flicker issue
function preventBackgroundScroll() {
    // Record current scroll position
    const scrollY = window.scrollY || document.documentElement.scrollTop;
    // Set scroll position to body
    document.body.style.position = 'fixed';
    document.body.style.top = `-${scrollY}px`;
    document.body.style.width = '100%';
    // Remove styles that may cause flickering
    document.body.style.overflow = 'visible';
    // Remove padding to prevent iOS background expansion
    document.body.style.paddingRight = '0px';
}

// New: Restore background scrolling - Fix iOS flicker issue
function restoreBackgroundScroll() {
    // Get saved scroll position
    const scrollY = Math.abs(parseInt(document.body.style.top || '0'));
    // Restore body styles - simplified to avoid flickering
