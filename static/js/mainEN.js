...
const UnifiedButtonManager = {
    ...
    // Prevent iOS highlight on touch
    initGlobalTouchHandlers() {
        // Prevent default iOS touch behavior
        document.addEventListener('touchstart', function(e) {
            // Only prevent for our custom buttons
            if (e.target.closest('.custom-btn, .option-btn, .quick-reply-btn')) {
                e.preventDefault();
            }
        }, { passive: false });
        ...
        document.addEventListener('touchstart', function(e) {
            const btn = e.target.closest('.custom-btn, .option-btn, .quick-reply-btn');
            if (btn) {
    ...
    selectButton(button, groupSelector) {
        ...
        this.activeButtons.set(button, true);
...
function clearStepsAfter(step) {
    ...
    if (startIndex === undefined || startIndex < 0) {
        ...
        if (lastFormIndex >= 0) {
            ...
            // Reset step index - this is the key fix
...
function bindEvents() {
    ...
    // Initialize unified button manager
    UnifiedButtonManager.initGlobalTouchHandlers();
}
...
$(document).ready(function () {
    ...
    // If there are pre-selected values, set corresponding state
    if (selectedFloor) {
...
function showServiceButtons() {
    ...
    const buttonsHTML = `
        <div class="service-buttons" id="serviceButtons">
            ${services.map(service => `
                <div class="service-button custom-btn" data-service="${service.value}">
    ...
    // Bind events with unified handler
    setTimeout(() => {
        $(document).off('click touchstart', '.service-button').on('click touchstart', '.service-button', function (e) {
            e.preventDefault();
            e.stopPropagation();
            
            // Use unified button manager
            UnifiedButtonManager.selectButton(this, '.service-button');
            
            const serviceType = $(this).data('service');
            setTimeout(() => selectService(serviceType), 50);
        });
    }, 100);
}
    ...
    if (currentService === 'office') {
        ...
        const formHTML = `
            ...
            <div class="form-group">
                ...
                <div class="location-select-btn custom-btn location-select-btn-office">
            ...
            <div class="quick-replies">
                <div class="quick-reply-btn custom-btn confirm-form-btn-office">
                    Next
                </div>
            </div>
        `;
        ...
        const formHTML = `
            ...
            <div class="form-group">
                <label class="form-label">Floor <span>*</span></label>
                <div class="floor-select-btn custom-btn floor-select-btn-dynamic">
            ...
            <div class="form-group">
                <label class="form-label">Location <span>*</span></label>
                <div class="location-select-btn custom-btn location-select-btn-dynamic">
                    <div class="location-display">
                        <span class="location-value">${locationDisplayText}</span>
                        <span class="location-arrow">▼</span>
                    </div>
            ...
            <div class="quick-replies">
                <div class="quick-reply-btn custom-btn confirm-form-btn-dynamic">
                    Next
                </div>
            </div>
        `;
...
function bindFormEvents() {
    ...
    $(document).off('click touchstart', '.location-select-btn-office');
    ...
    $(document).off('click touchstart', '.confirm-form-btn-office');
    $(document).off('click touchstart', '.floor-select-btn-dynamic');
    $(document).off('click touchstart', '.location-select-btn-dynamic');
    $(document).off('click touchstart', '.confirm-form-btn-dynamic');
    ...
    if (currentService === 'office') {
        ...
        $(document).on('click touchstart', '.location-select-btn-office', showLocationModal);
        ...
        $(document).on('click touchstart', '.confirm-form-btn-office', confirmForm);
    } else {
        $(document).on('click touchstart', '.floor-select-btn-dynamic', showFloorModal);
        $(document).on('click touchstart', '.location-select-btn-dynamic', showLocationModal);
        $(document).on('click touchstart', '.confirm-form-btn-dynamic', confirmForm);
...
function askForPhoto() {
    ...
    if (hasPhoto) {
        ...
        photoHTML = `
            ...
            <div class="quick-replies" style="margin-top: 18px;">
                <div class="quick-reply-btn custom-btn skip-photo-btn-dynamic">
                    Remove
                </div>
                <div class="quick-reply-btn custom-btn confirm-photo-btn-dynamic">
                    Next
                </div>
            </div>
        `;
        ...
        photoHTML = `
            <div class="upload-area upload-area-dynamic custom-btn">
    ...
    // Bind events
    setTimeout(() => {
        $(document).off('click touchstart', '.upload-area-dynamic').on('click touchstart', '.upload-area-dynamic', handleUploadClick);
        $(document).off('click touchstart', '.skip-photo-btn-dynamic').on('click touchstart', '.skip-photo-btn-dynamic', skipPhoto);
        $(document).off('click touchstart', '.confirm-photo-btn-dynamic').on('click touchstart', '.confirm-photo-btn-dynamic', confirmPhoto);
...
function skipPhoto() {
    // 在現有代碼中，確保不會預設選中
    $('.skip-photo-btn-dynamic').removeClass('selected');
...
function showConfirmation() {
    ...
    const confirmationHTML = `
        ...
        <div class="quick-replies">
            <div class="quick-reply-btn custom-btn edit-info-btn">
                ✏️ Edit
            </div>
            <div class="quick-reply-btn custom-btn submit-report-btn" style="background: #4a90e2 !important; color: white !important;">
                Submit
            </div>
        </div>
    `;
    ...
    // Bind events
    setTimeout(() => {
        $(document).off('click touchstart', '.edit-info-btn').on('click touchstart', '.edit-info-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).off('click touchstart', '.submit-report-btn').on('click touchstart', '.submit-report-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();
            submitReport();
        });
...
function showSuccessPage() {
    ...
    const successHTML = `
        <div class="success-container">
            ...
            <div class="quick-replies">
                <div class="quick-reply-btn custom-btn new-report-btn-dynamic">
                    New Report
                </div>
            </div>
        </div>
    ...
    $(document).off('click touchstart', '.new-report-btn-dynamic').on('click touchstart', '.new-report-btn-dynamic', function () {
...
function showFloorModal() {
    ...
    const floorOptionsHTML = floors.map(floor => {
        ...
        return `
            <div class="floor-option-btn option-btn custom-btn ${isSelected ? 'selected' : ''}"
                    data-floor="${floor}">
                ${displayFloor}
            </div>
        `;
    ...
    $(document).off('click touchstart', '.floor-option-btn').on('click touchstart', '.floor-option-btn', function (e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Use unified button manager
        UnifiedButtonManager.selectButton(this, '.floor-option-btn');
        
        const floor = $(this).data('floor');
        setTimeout(() => selectFloor(floor), 50);
    });

    // Show modal with modern approach
    showModernModal('floorModal');
}
...
function showLocationModal() {
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <div class="custom-location-confirm-btn custom-btn custom-location-confirm-btn-dynamic">
                Confirm
            </div>
        </div>
    `;
    ...
    const locationOptionsHTML = locations.map(location => {
        ...
        return `
            <div class="location-option-btn option-btn custom-btn ${isSelected ? 'selected' : ''}"
    ...
    $(document).off('click touchstart', '.location-option-btn').on('click touchstart', '.location-option-btn', function (e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Use unified button manager
        UnifiedButtonManager.selectButton(this, '.location-option-btn');
        
        const locationId = $(this).data('location');
        setTimeout(() => selectLocation(locationId), 50);
    });

    $(document).off('click touchstart', '.custom-location-confirm-btn-dynamic').on('click touchstart', '.custom-location-confirm-btn-dynamic', confirmCustomLocation);
    ...
    // Show modal with modern approach
    showModernModal('locationModal');
}

// Modern modal display function
function showModernModal(modalId) {
...
function selectLocation(locationId) {
    ...
    closeModernModal('locationModal');
...
function closeImageModal() {
    closeModernModal('imageModal');
}

function previewUploadedImage() {
    if (uploadedFilePreview) {
        $('#modalImage').attr('src', uploadedFilePreview);
        showModernModal('imageModal');
    }
}
