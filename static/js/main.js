...
// iOS 專用「硬切斷」模態框打開函數
function openModalIOSHardFix(modalEl) {
    if (!modalEl) return;

    // 非 iOS Safari：直接顯示，完全不影響
    if (!isIOSSafari()) {
    ...
    // iOS Safari：硬切斷 touch lifecycle
    console.log('iOS Safari 檢測到，啟用硬切斷修復');
    ...
    // ④ 等 touch lifecycle 完全結束（關鍵）
    setTimeout(() => {
        modalEl.classList.add('is-visible');
        console.log('iOS Safari modal 可見性已延遲設置');
    }, 50); // ★ 這 50ms 就是解答
}

// 統一的模態框關閉函數
function closeModalHardFix($modal) {
...
function showFloorModal() {
    ...
    openModalIOSHardFix(floorModal);
}
...
function showLocationModal() {
    ...
    openModalIOSHardFix(locationModal);
}
...
function closeFloorModal() {
    closeModalHardFix($('#floorModal'));
}

function closeLocationModal() {
    closeModalHardFix($('#locationModal'));
}

function closeImageModal() {
    closeModalHardFix($('#imageModal'));
}

// 修改現有的 closeModal 函數來使用新的修復版本
function closeModal($modal) {
    closeModalHardFix($modal);
}
...
function previewUploadedImage() {
    if (uploadedFilePreview) {
        ...
        openModalIOSHardFix(imageModal);
    }
}
