...
const ButtonStateManager = {
    ...
    initButtonGroup(groupId, selector, singleSelect = true) {
        ...
        $(document).off('click', selector).on('click', selector, (e) => {
            // 防止 iOS 在彈窗關閉瞬間誤觸到底層元素
            e.preventDefault();
            this.handleButtonClick(groupId, $(e.currentTarget));
        });

        // 初始化樣式 (進入此步驟時清理舊狀態)
        $(selector).removeClass('selected active');
    },

    // 處理點擊 (這是解決返回修改問題的核心)
    handleButtonClick(groupId, $btn) {
        ...
        // 2. 根據點擊的是「樓層」還是「地點」執行對應動作
        if (groupId === 'floor-buttons') {
            const floor = $btn.data('floor');
            if (typeof selectFloor === 'function') {
                selectFloor(floor); // 執行原本的填入數值邏輯
            }
            // 【核心修正】點完樓層，延遲 150ms 自動關閉模態框
            setTimeout(() => {
                $('#floorModal').fadeOut(200);
            }, 150);

        } else if (groupId === 'location-buttons') {
            const location = $btn.data('location');
            if (typeof selectLocation === 'function') {
                selectLocation(location); // 執行原本的填入數值邏輯
            }
            // 【核心修正】點完地點，延遲 150ms 自動關閉模態框
            setTimeout(() => {
                $('#locationModal').fadeOut(200);
            }, 150);
        }
    },
...
function submitReport() {
    ...
    // 【核心修正 1】防止重複提交 (iOS 優化)
    const $submitBtn = $('.confirm-send-btn');
    if ($submitBtn.prop('disabled')) {
        console.log('按鈕已停用，跳過重複提交');
        return;
    }

    // 檢查狀態
    if (currentStep === 'submitting' || currentStep === 'completed') {
        console.log('已經在提交中，跳過');
        return;
    }

    // 【核心修正 2】立即停用按鈕，解決 iOS 26.2 觸控殘留與重複觸發
    ...
    if (!lineDescription || lineDescription.trim() === '' ||
    ...
    processPromise
        ...
        .then(() => {
            ...
            setTimeout(() => {
                const $progress = $(`#${progressId}`);
                if ($progress.length > 0) $progress.remove();
                setStep('completed');
            }, 1000);
        })
        .catch(error => {
