...
// ==================== 初始化應用 ====================
// 1. 在 $(document).ready 外部定義這個通用函數，方便重複調用
function initTouchFeedback() {
    // 列出所有需要點擊回饋的按鈕 class
    const touchSelectors = '.service-button, .quick-reply-btn, .floor-option-btn, .location-option-btn, .submit-btn, .modal-close';

    $(document).on('touchstart', touchSelectors, function () {
        $(this).addClass('is-active');
    });

    $(document).on('touchend touchcancel', touchSelectors, function () {
        const $el = $(this);
        // 延遲 80ms 移除，確保使用者看得到變色，但又能及時消失
        setTimeout(() => {
...
$(document).ready(function () {
    ...
    // B. 您原本的邏輯：初始化按鈕狀態 (這裡移除原本的 active 關鍵字，統一用 selected 管理長期狀態)
    $('.floor-option-btn, .location-option-btn').removeClass('selected is-active');
    ...
    // C. 針對樓層按鈕的點擊處理 (如果原本 bindEvents 裡沒寫，就加在這裡)
    // 使用 $(document).on 是為了確保「動態生成」的按鈕也能被點擊
    $(document).on('click', '.floor-option-btn', function () {
        // 當點擊發生時，立刻強制移除所有按鈕的觸摸 active 狀態，防止殘留到下一頁
        $('.is-active').removeClass('is-active');

        // 執行原本的業務邏輯...
        console.log("Floor selected:", $(this).data('floor'));
    });
