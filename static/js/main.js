...
window.IOSSpatialBugShield = (function () {
    ...
    function deepCleanAfterModalClose() {
        if (!isIOSSafari) return;

        console.log('🛡️ Deep cleaning after modal close...');

        // 1. 強制清理所有可能的選取狀態
        document.querySelectorAll('.floor-option-btn.selected, .location-option-btn.selected').forEach(btn => {
            // 檢查這個按鈕是否應該真的被選取
            const shouldBeSelected = btn.hasAttribute('data-selected-by-user');
            if (!shouldBeSelected) {
                btn.classList.remove('selected');
                btn.classList.add('force-unselected');
            }
        });

        // 2. 特別清理通報表單上的按鈕
        document.querySelectorAll('.floor-select-btn, .location-select-btn').forEach(btn => {
            btn.classList.remove('selected');
            btn.classList.add('force-unselected');

            // 3. 強制重繪，打斷 iOS 的狀態記憶
            btn.style.opacity = '0.999';
            setTimeout(() => {
                btn.style.opacity = '';
            }, 10);
        });
        ...
        // 4. 給 iOS 一個完整的重繪週期
        setTimeout(() => {
            document.querySelectorAll('.force-unselected').forEach(el => {
                el.classList.remove('force-unselected');
            });
        }, 100);
    }

    // 更新 closeModalSafely 函數
    function closeModalSafely(closeModalFn) {
        if (!isIOSSafari) {
            closeModalFn();
            return;
        ...
        console.log('🛡️ Closing modal safely for iOS...');

        forceBlur();

        // 標記 modal 正在關閉
        const modal = document.querySelector('.modal.show, .modal[style*="display"]');
        if (modal) {
    ...
    IOSSpatialBugShield.openModalSafely(() => {
        ...
        $(document).off('click', '.modal-floor-btn').on('click', '.modal-floor-btn', function (e) {
            ...
            IOSSpatialBugShield.closeModalSafely(() => {
                ...
                setTimeout(() => {
                    // 立即清理通報表單按鈕的狀態
                    $('.floor-select-btn-dynamic').removeClass('selected');
        ...
        $(document).off('click', '.modal-location-btn').on('click', '.modal-location-btn', function (e) {
            ...
            IOSSpatialBugShield.closeModalSafely(() => {
                ...
                setTimeout(() => {
                    // 立即清理通報表單按鈕的狀態
                    if (currentService === 'office') {
                        $('.location-select-btn-office').removeClass('selected');
                    } else {
                        $('.location-select-btn-dynamic').removeClass('selected');
                    }
