...
function showFloorModal() {
    ...
    // 使用模態框管理器打開 - 修改這行
    ModalManager.openModal($('#floorModal'));
}
...
function showLocationModal() {
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <button class="custom-location-confirm-btn custom-location-confirm-btn-dynamic" type="button">
                確認自訂地點
            </button>
        </div>
    `;
    ...
    const locationOptionsHTML = locations.map(location => {
        ...
        return `
            <button class="location-option-btn location-option-btn-dynamic ${isSelected ? 'selected' : ''}"
                    data-location="${location.id}" type="button">
                ${location.label}
            </button>
        `;
    }).join('');
    ...
    // 使用模態框管理器打開 - 修改這行
    ModalManager.openModal($('#locationModal'));
}
...
function closeFloorModal() {
    // 修改這行
    ModalManager.closeModal($('#floorModal'));
}

function closeLocationModal() {
    // 修改這行
    ModalManager.closeModal($('#locationModal'));
}

function closeImageModal() {
    // 修改這行
    ModalManager.closeModal($('#imageModal'));
}
