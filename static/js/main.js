...
function showModalWithDelay(modalElement) {
    if(document.activeElement) document.activeElement.blur(); // 消除焦點
    document.body.style.overflow = 'hidden';
    modalElement.style.pointerEvents = 'none'; // 防穿透護盾開啟
    modalElement.style.display = 'flex';
    setTimeout(() => modalElement.style.pointerEvents = 'auto', 350); // 動畫結束後解除護盾
}

function showFloorModal() {
    DOM.select('#floorModalTitle').textContent = `選擇${APP_CONFIG[currentService].name_tw}樓層`;
    const floors = [...new Set(APP_CONFIG[currentService].floors.split(',').map(s => s.trim()).filter(s => s))];
...
function showLocationModal() {
    ...
    const locs = [...new Set(APP_CONFIG[currentService].locations_tw)];
    const customHTML = `
