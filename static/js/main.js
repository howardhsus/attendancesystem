...
window.IOSSpatialBugShield = (function () {
  ...
  function openModalSafely(openFn) {
    if (!isIOSSafari) return openFn();
    breakSpatialState();
    requestAnimationFrame(openFn);
  }

  return { openModalSafely, isIOSSafari };
})();
...
$(document).ready(function () {
    ...
    // 初始化按鈕狀態
    $('.floor-option-btn, .location-option-btn').removeClass('selected');
...
function showFloorModal() {
    ...
    // 最乾淨的選取邏輯
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function () {
...
function showLocationModal() {
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <button class="custom-location-confirm-btn">
                確認自訂地點
            </button>
        </div>
    `;
    ...
    // 最乾淨的選取邏輯
    $(document).off('click', '.location-option-btn').on('click', '.location-option-btn', function () {
    ...
    $(document).off('click', '.custom-location-confirm-btn').on('click', '.custom-location-confirm-btn', confirmCustomLocation);

    $(document).off('focus blur', '.custom-location-input')
        .on('focus', '.custom-location-input', function () {
            if ($(this).hasClass('placeholder-active')) {
                $(this).removeClass('placeholder-active');
                $(this).val('');
            }
        })
        .on('blur', '.custom-location-input', function () {
            if (!$(this).val().trim()) {
