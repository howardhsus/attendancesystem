...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function (e) {
        ...
        console.log('樓層按鈕點擊，樓層:', floor, '按鈕:', $btn);
    ...
    $(document).off('click', '.location-option-btn').on('click', '.location-option-btn', function (e) {
        ...
        console.log('地點按鈕點擊，地點:', locationId, '按鈕:', $btn);
...
function showFloorModal() {
    console.log('顯示樓層模態框，當前服務:', currentService, '已選樓層:', selectedFloor);
    ...
    const floorOptionsHTML = floors.map(floor => {
        ...
        return `
            <button class="floor-option-btn" data-floor="${floor}" ${isSelected ? 'style="border-color: #4a90e2; background: rgba(74, 144, 226, 0.1); color: #4a90e2; font-weight: 600;"' : ''}>
                ${displayFloor}
            </button>
        `;
    }).join('');
    ...
    // 綁定模態框內按鈕的事件 - 直接執行選擇邏輯
    $('#floorOptions .floor-option-btn').off('click').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        console.log('模態框樓層點擊，樓層:', floor);

        // 清除模態框內所有按鈕的 selected 狀態
        $('#floorOptions .floor-option-btn').removeClass('selected');
        $('#floorOptions .floor-option-btn').css({
            'border-color': '#e0e0e0',
            'background': 'white',
            'color': '#333',
            'font-weight': '500'
...
function showLocationModal() {
    console.log('顯示地點模態框，當前服務:', currentService, '已選地點:', selectedLocation);
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <div class="custom-location-group" id="modalCustomLocationGroup">
                ...
                <input type="text" class="custom-location-input" id="modalCustomLocationInput"
                       placeholder="輸入自訂地點" value="${customLocation || ''}">
            </div>
            <button class="custom-location-confirm-btn" id="modalCustomLocationConfirm">
                確認自訂地點
            </button>
        </div>
    `;
    ...
    const locationOptionsHTML = locations.map(location => {
        ...
        return `
            <button class="location-option-btn" data-location="${location.id}" ${isSelected ? 'style="border-color: #4a90e2; background: rgba(74, 144, 226, 0.1); color: #4a90e2; font-weight: 600;"' : ''}>
                ${location.label}
            </button>
        `;
    }).join('');
    ...
    // 綁定地點按鈕事件 - 直接執行選擇邏輯
    $('#locationOptions .location-option-btn').off('click').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        console.log('模態框地點點擊，地點ID:', locationId);

        // 清除模態框內所有按鈕的 selected 狀態
        $('#locationOptions .location-option-btn').removeClass('selected');
        $('#locationOptions .location-option-btn').css({
            'border-color': '#e0e0e0',
            'background': 'white',
            'color': '#333',
            'font-weight': '500'
    ...
    // 綁定自訂地點確認按鈕
    $('#modalCustomLocationConfirm').off('click').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
...
function closeModal($modal) {
    ...
    if ($modal.attr('id') === 'floorModal') {
        $('#floorOptions .floor-option-btn').off('click');
    } else if ($modal.attr('id') === 'locationModal') {
        $('#locationOptions .location-option-btn').off('click');
        $('#modalCustomLocationConfirm').off('click');
    }
}
