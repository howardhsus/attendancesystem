...
const ButtonStateManager = {
    ...
    handleButtonClick(groupId, $button) {
        ...
        // 防抖處理：防止快速雙擊
        const now = Date.now();
        if (now - this.lastClickTimes[groupId] < 150) {
            return;
        }
        this.lastClickTimes[groupId] = now;

        const buttonId = $button.data('id') || $button.attr('id') || $button.index();

        // 清除同組的其他按鈕狀態
        $(group.selector).not($button).each((i, btn) => {
            $(btn).removeClass('selected active');
            btn.blur();
        });
...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function (e) {
        ...
        // 防雙擊
        const now = Date.now();
        if (now - lastFloorClickTime < 300) return;
        lastFloorClickTime = now;

        const $btn = $(this);
        const floor = $btn.data('floor');

        console.log('樓層按鈕點擊:', floor);
    ...
    $(document).off('click', '.location-option-btn').on('click', '.location-option-btn', function (e) {
        ...
        // 防雙擊
        const now = Date.now();
        if (now - lastLocationClickTime < 300) return;
        lastLocationClickTime = now;

        const $btn = $(this);
        const locationId = $btn.data('location');

        console.log('地點按鈕點擊:', locationId);
...
$(document).ready(function () {
    ...
    $('.floor-option-btn, .location-option-btn').blur();
...
function showFloorModal() {
    ...
    const floorOptionsHTML = floors.map(floor => {
        ...
        return `
            <button class="floor-option-btn floor-option-btn-modal ${isSelected ? 'selected' : ''}"
                    data-floor="${floor}">
                ${displayFloor}
            </button>
        `;
    ...
    // 綁定事件 - 使用獨立的事件處理器避免衝突
    $(document).off('click', '.floor-option-btn-modal').on('click', '.floor-option-btn-modal', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        console.log('模態框樓層按鈕點擊:', floor);

        // 清除所有狀態
        $('.floor-option-btn-modal').removeClass('selected active');
...
function showLocationModal() {
    ...
    // 綁定事件 - 使用獨立的事件處理器避免衝突
    $(document).off('click', '.location-option-btn-modal').on('click', '.location-option-btn-modal', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        console.log('模態框地點按鈕點擊:', locationId);

        // 清除所有狀態
        $('.location-option-btn-modal').removeClass('selected active');
    ...
    $(document).off('click', '.custom-location-confirm-btn-modal').on('click', '.custom-location-confirm-btn-modal', function (e) {
        e.preventDefault();
        e.stopPropagation();
...
function selectFloor(floor) {
    ...
    console.log('選擇樓層:', floor, '顯示:', displayFloor);
...
function selectLocation(locationId) {
    ...
    console.log('選擇地點:', locationId, '標籤:', locationLabel);
...
// 統一的模態框關閉函數 - 修復 iOS 狀態殘留
function closeModal($modal) {
    console.log('關閉模態框:', $modal.attr('id'));
    ...
    // 清除模態框內按鈕的事件綁定
    if ($modal.attr('id') === 'floorModal') {
        $(document).off('click', '.floor-option-btn-modal');
    } else if ($modal.attr('id') === 'locationModal') {
        $(document).off('click', '.location-option-btn-modal');
        $(document).off('click', '.custom-location-confirm-btn-modal');
        $(document).off('focus blur', '.custom-location-input-modal');
    }
}
