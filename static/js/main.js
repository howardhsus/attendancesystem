...
function skipPhoto() {
    ...
    // 修復問題2：清除按鈕的預設選取狀態
    $('.skip-photo-btn-dynamic').removeClass('selected active');
...
function showFloorModal() {
    ...
    const floorOptionsHTML = floors.map(floor => {
        ...
        return `
            <button class="floor-option-btn floor-option-btn-modal ios-fix"
                    data-floor="${floor}"
                    data-selected="${dataSelected}"
                    ${isSelected ? 'style="border-color: #4a90e2 !important; background: rgba(74, 144, 226, 0.1) !important; color: #4a90e2 !important; font-weight: 600 !important;"' : ''}>
                ${displayFloor}
            </button>
        `;
    }).join('');
    ...
    // 使用 JavaScript 來管理狀態，而不是依賴 CSS 類
    setTimeout(() => {
        const selectedFloorValue = selectedFloor;
        if (selectedFloorValue) {
            // 使用數據屬性來設置樣式
            $(`.floor-option-btn-modal[data-floor="${selectedFloorValue}"]`).each(function () {
                $(this).css({
                    'border-color': '#4a90e2',
    ...
    $(document).off('click', '.floor-option-btn-modal').on('click', '.floor-option-btn-modal', function () {
...
function showLocationModal() {
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <div class="custom-location-group" id="modalCustomLocationGroup">
                ...
                <input type="text" class="custom-location-input custom-location-input-modal ${customLocation ? '' : 'placeholder-active'}"
                       id="modalCustomLocationInput"
                       placeholder="輸入自訂地點"
                       value="${customLocation || ''}">
            </div>
            <button class="custom-location-confirm-btn custom-location-confirm-btn-modal">
                確認自訂地點
            </button>
        </div>
    `;
    ...
    const locationOptionsHTML = locations.map(location => {
        // 關鍵修復：使用全域變數 selectedLocation 來判斷是否選中
        const isSelected = (selectedLocation === location.id);
        return `
            <button class="location-option-btn location-option-btn-modal ${isSelected ? 'selected' : ''}"
    ...
    // 移除舊的事件監聽器
    $(document).off('click', '.location-option-btn-modal').on('click', '.location-option-btn-modal', function () {
        const locationId = $(this).data('location');
        selectLocation(locationId);
    });

    $(document).off('click', '.custom-location-confirm-btn-modal').on('click', '.custom-location-confirm-btn-modal', confirmCustomLocation);

    $(document).off('focus blur', '.custom-location-input-modal')
        .on('focus', '.custom-location-input-modal', function () {
            if ($(this).hasClass('placeholder-active')) {
                $(this).removeClass('placeholder-active');
                $(this).val('');
            }
        })
        .on('blur', '.custom-location-input-modal', function () {
            if (!$(this).val().trim()) {
...
function selectFloor(floor) {
    ...
    if (currentService === 'shopping' || currentService === 'observatory') {
        ...
        // 確保按鈕樣式正確
        $('.floor-select-btn-dynamic').removeClass('placeholder-active').addClass('has-selection');
    }

    // 更新模態框內的按鈕狀態
    $('#floorModal .floor-option-btn-modal').removeClass('selected');
    $(`#floorModal .floor-option-btn-modal[data-floor="${floor}"]`).addClass('selected');
...
function selectLocation(locationId) {
    ...
    if (currentService === 'office') {
        ...
        $('.location-select-btn-office').removeClass('placeholder-active').addClass('has-selection');
    } else {
        $('.location-select-btn-dynamic .location-value').text(locationLabel);
        $('.location-select-btn-dynamic').removeClass('placeholder-active').addClass('has-selection');
    }

    // 更新模態框內的按鈕狀態
    $('#locationModal .location-option-btn-modal').removeClass('selected');
    $(`#locationModal .location-option-btn-modal[data-location="${locationId}"]`).addClass('selected');
...
function updateFormDisplay() {
    // 更新樓層顯示
    if (selectedFloor) {
        const displayFloor = `${selectedFloor}F`;
        if (currentService === 'shopping' || currentService === 'observatory') {
            $('.floor-select-btn-dynamic .floor-value').text(displayFloor);
            $('.floor-select-btn-dynamic').removeClass('placeholder-active').addClass('has-selection');
        }
    ...
    if (selectedLocation || customLocation) {
        ...
        if (currentService === 'office') {
            $('.location-select-btn-office .location-value').text(locationText);
            $('.location-select-btn-office').removeClass('placeholder-active').addClass('has-selection');
        } else {
            $('.location-select-btn-dynamic .location-value').text(locationText);
            $('.location-select-btn-dynamic').removeClass('placeholder-active').addClass('has-selection');
        }
    }
}
...
function preventBackgroundScroll() {
    ...
    // 移除 padding，防止 iOS 背景擴展
    document.body.style.paddingRight = '0px';
}

// 新增：恢復背景滾動 - 修復 iOS 閃爍問題
