...
const CrossPlatformManager = {
    ...
    applyIOSFixes: function () {
        ...
        document.addEventListener('touchstart', function (e) {
            if (e.target.type === 'text' || e.target.type === 'textarea' || e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
                return;
            }
            e.preventDefault();
        }, { passive: false });
    ...
    // 安全的點擊處理器（解決 iOS 狀態洩漏問題）
    safeClickHandler: function (callback, options = {}) {
        return function (e) {
            // 防止快速雙擊
            const now = Date.now();
            ...
            if (CrossPlatformManager.isIOS()) {
                ...
                if (options.clearOthers) {
                    $(options.clearOthers).removeClass('active selected');
                }
    ...
    // 修復模態框滾動
    fixModalScroll: function (modalId) {
...
const ButtonStateManager = {
    ...
    initButtonGroup(groupId, selector, singleSelect = true) {
        ...
        // 綁定事件 - 使用跨平台安全的處理器
        $(document).off('click', selector).on('click', selector,
            CrossPlatformManager.safeClickHandler((e) => {
                this.handleButtonClick(groupId, $(e.currentTarget));
            }, {
                clearOthers: selector,
                delay: 100
            })
        );
...
function setupStableButtonHandlers() {
    // 防止事件重複綁定
    $(document).off('click.stableHandlers');

    // 樓層選擇按鈕
    $(document).on('click.stableHandlers', '.floor-option-btn',
        CrossPlatformManager.safeClickHandler(function (e) {
            const $btn = $(this);
            ...
            clearOthers: '.floor-option-btn',
            delay: 100
        })
    );

    // 地點選擇按鈕
    $(document).on('click.stableHandlers', '.location-option-btn',
        CrossPlatformManager.safeClickHandler(function (e) {
            const $btn = $(this);
            ...
            clearOthers: '.location-option-btn',
            delay: 100
        })
    );

    // 服務選擇按鈕
    $(document).on('click.stableHandlers', '.service-button',
        CrossPlatformManager.safeClickHandler(function (e) {
            const $btn = $(this);
            ...
            clearOthers: '.service-button',
            delay: 100
        })
    );

    // 快速回復按鈕
    $(document).on('click.stableHandlers', '.quick-reply-btn',
        CrossPlatformManager.safeClickHandler(function (e) {
            const $btn = $(this);
    ...
    // 上傳區域
    $(document).on('click.stableHandlers', '.upload-area-dynamic',
        CrossPlatformManager.safeClickHandler(function (e) {
            handleUploadClick();
        }, { delay: 100 })
    );
...
function bindEvents() {
    // 清除舊的事件綁定
    $('#reloadLogo').off('click');
    $('#closeImageModalBtn').off('click');
    $('#closeFloorModalBtn').off('click');
    $('#closeLocationModalBtn').off('click');
    $('.modal').off('click');
    $('.report-label').off('click');

    // 重新綁定事件
    $('#reloadLogo').on('click', function () {
        location.reload();
    });

    $('#closeImageModalBtn').on('click', closeImageModal);
    $('#closeFloorModalBtn').on('click', closeFloorModal);
    $('#closeLocationModalBtn').on('click', closeLocationModal);
    ...
    // 切換到英文版按鈕
    $('.report-label').on('click', function () {
...
$(document).ready(function () {
    // 初始化跨平台修復
    CrossPlatformManager.initPlatformFixes();
...
function showServiceButtons() {
    ...
    // 事件綁定已在 setupStableButtonHandlers 中處理
}
...
function bindFormEvents() {
    // 移除所有舊的事件綁定
    $(document).off('focus.formEvents');
    $(document).off('blur.formEvents');
    $(document).off('input.formEvents');
    $(document).off('click.formEvents');

    if (currentService === 'office') {
        // 辦公大樓表單事件
        $(document).on('focus.formEvents', '.floor-input-office', handleFloorFocus);
        $(document).on('blur.formEvents', '.floor-input-office', handleOfficeFloorBlur);
        $(document).on('input.formEvents', '.floor-input-office', handleOfficeFloorInput);

        $(document).on('focus.formEvents', '.description-input-office', handleDescriptionFocus);
        $(document).on('blur.formEvents', '.description-input-office', handleDescriptionBlur);
        $(document).on('input.formEvents', '.description-input-office', handleDescriptionInput);

        // 地點選擇按鈕
        $(document).on('click.formEvents', '.location-select-btn-office',
            CrossPlatformManager.safeClickHandler(function (e) {
                showLocationModal();
            }, { delay: 100 })
        );
        ...
        // 購物中心/觀景台表單事件
        $(document).on('focus.formEvents', '.description-input-dynamic', handleDescriptionFocus);
        $(document).on('blur.formEvents', '.description-input-dynamic', handleDescriptionBlur);
        $(document).on('input.formEvents', '.description-input-dynamic', handleDescriptionInput);

        // 樓層選擇按鈕
        $(document).on('click.formEvents', '.floor-select-btn-dynamic',
            CrossPlatformManager.safeClickHandler(function (e) {
                showFloorModal();
            }, { delay: 100 })
        );
        ...
        // 地點選擇按鈕
        $(document).on('click.formEvents', '.location-select-btn-dynamic',
            CrossPlatformManager.safeClickHandler(function (e) {
                showLocationModal();
            }, { delay: 100 })
        );
...
function showFloorModal() {
    ...
    // 清除舊的事件綁定
    $(document).off('click.floorModal');

    // 綁定模態框內的按鈕事件
    $(document).on('click.floorModal', '#floorModal .floor-option-btn-dynamic',
        CrossPlatformManager.safeClickHandler(function () {
            ...
            clearOthers: '#floorModal .floor-option-btn-dynamic',
            delay: 100
        })
    );

    // 應用 iOS 修復
    CrossPlatformManager.fixModalScroll('#floorModal');
...
function showLocationModal() {
    ...
    // 清除舊的事件綁定
    $(document).off('click.locationModal');
    $(document).off('focus.locationModal');
    $(document).off('blur.locationModal');
    $(document).off('input.locationModal');

    // 綁定模態框內的按鈕事件
    $(document).on('click.locationModal', '#locationModal .location-option-btn-dynamic',
        CrossPlatformManager.safeClickHandler(function () {
            ...
            // 清除自訂地點輸入
            $('#modalCustomLocationInput').val('').removeClass('placeholder-active');

            selectLocation(locationId);
        }, {
            clearOthers: '#locationModal .location-option-btn-dynamic',
            delay: 100
        })
    );

    // 自訂地點確認按鈕
    $(document).on('click.locationModal', '#locationModal .custom-location-confirm-btn-dynamic',
        CrossPlatformManager.safeClickHandler(function () {
    ...
    // 自訂地點輸入框事件
    $(document).on('focus.locationModal', '#modalCustomLocationInput', function () {
    ...
    $(document).on('blur.locationModal', '#modalCustomLocationInput', function () {
        if (!$(this).val().trim()) {
    ...
    // 輸入時清除預選地點
    $(document).on('input.locationModal', '#modalCustomLocationInput', function () {
        $('#locationModal .location-option-btn-dynamic').removeClass('selected');
    });

    // 應用 iOS 修復
    CrossPlatformManager.fixModalScroll('#locationModal');
