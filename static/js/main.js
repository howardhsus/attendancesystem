...
const iOSSafariFix = (function () {
    ...
    console.log('iOS Safari 物理位置防護系統已啟用');

    // 儲存所有受保護的按鈕
    const protectedButtons = new Set();

    // 清除物理位置記憶
    function clearPhysicalMemory() {
        // 強制所有按鈕失去焦點
        ...
        breaker.className = 'ios-break-layer';
      ...
      pointer-events: none;
      opacity: 0;
      animation: breakFlash 0.3s ease;
    `;

        document.body.appendChild(breaker);

        // 強制重排
        breaker.offsetHeight;
    ...
    function protectButton(button) {
        if (!button || protectedButtons.has(button)) return button;

        protectedButtons.add(button);

        // 添加保護類別
        button.classList.add('ios-protected');
        ...
        // 創建新的點擊處理函數
        const safeClickHandler = function (e) {
            if (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            allButtons.forEach(btn => {
                btn.classList.remove('active', 'ios-active');
                if (btn !== button) {
                    btn.blur();
                }
            });
            ...
            setTimeout(() => {
                ...
                if (button.getAttribute('onclick')) {
                    try {
                        ...
                        console.warn('執行 onclick 時出錯:', err);
                    }
                }
            }, 20);
        ...
        // 移除所有事件監聽器
        const newElement = newButton;
        newElement.onclick = null;
        newElement.onmousedown = null;
        newElement.ontouchstart = null;

        // 重新綁定事件
        newElement.addEventListener('click', safeClickHandler, true);
        newElement.addEventListener('touchend', safeClickHandler, true);
    ...
    // 保護模態框
    function protectModal(modal) {
        if (!modal) return;

        // 保護模態框內所有按鈕
        const buttons = modal.querySelectorAll('button, [role="button"]');
        buttons.forEach(btn => protectButton(btn));
                    ...
                    if (modal.style.display === 'none' || modal.style.visibility === 'hidden') {
                        ...
                        setTimeout(() => {
                            ...
                            const mainButtons = document.querySelectorAll(
                                '.floor-select-btn, .location-select-btn, .service-button, .quick-reply-btn'
                            );
    ...
    function init() {
        ...
        document.addEventListener('touchend', (e) => {
            // 延遲清除，確保在 iOS 渲染完成後
            setTimeout(clearPhysicalMemory, 100);
        }, true);
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) { // ELEMENT_NODE
                        ...
                        if (node.matches && (node.matches('button') || node.matches('[role="button"]'))) {
                            setTimeout(() => protectButton(node), 10);
                        }
                        ...
                        if (node.querySelectorAll) {
                            ...
                            buttons.forEach(btn => {
                                setTimeout(() => protectButton(btn), 10);
                            });
                        }

                        // 檢查新增的模態框
                        if (node.matches && node.matches('.modal')) {
                            setTimeout(() => protectModal(node), 10);
                        }

                        if (node.querySelectorAll) {
                            const modals = node.querySelectorAll('.modal');
                            modals.forEach(modal => {
                                setTimeout(() => protectModal(modal), 10);
                            });
                        }
                    }
                });
...
const ButtonStateManager = {
    ...
    initButtonGroup(groupId, selector, singleSelect = true) {
        ...
        // 綁定事件 - 修復 iOS Safari 問題
        $(document).off('click', selector).on('click', selector, (e) => {
            e.preventDefault();
            e.stopPropagation();

            // iOS Safari 保護
            if (iOSSafariFix.isIOSSafari) {
                iOSSafariFix.clearPhysicalMemory();
            }
        ...
        $(selector).each((index, btn) => {
            $(btn).removeClass('selected active ios-active');
            // 確保所有按鈕都有正確的屬性
            $(btn).attr('data-ios-protected', 'true');

            // 保護按鈕
            if (iOSSafariFix.isIOSSafari) {
                iOSSafariFix.protectButton(btn);
            }
        });
    },
    ...
    handleButtonClick(groupId, $button) {
        ...
        if (group.singleSelect) {
            // 單選模式：先清除所有，再選中當前
            $(group.selector).removeClass('selected active ios-active');
...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.service-button').on('click', '.service-button', function (e) {
        ...
        if (iOSSafariFix.isIOSSafari) {
            iOSSafariFix.clearPhysicalMemory();
            iOSSafariFix.protectButton(this);
        }

        // 清除所有按鈕的 active 狀態
        $('button, [role="button"]').removeClass('active ios-active');
    ...
    // 樓層選擇按鈕
    $(document).off('click', '.floor-select-btn-dynamic, .floor-select-btn-office').on('click', '.floor-select-btn-dynamic, .floor-select-btn-office', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // 地點選擇按鈕
    $(document).off('click', '.location-select-btn-dynamic, .location-select-btn-office').on('click', '.location-select-btn-dynamic, .location-select-btn-office', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.floor-option-btn').removeClass('selected active ios-active');

        // 2. 設置當前按鈕狀態 - 跨平台修復
        $btn.addClass('selected');

        // 3. 立即移除焦點，防止 iOS 記憶狀態
        setTimeout(() => {
            $btn.blur();
        }, 0);

        // 4. 短暫的 active 狀態（視覺反饋）
        $btn.addClass('active');
        setTimeout(() => {
    ...
    $(document).off('click', '.location-option-btn').on('click', '.location-option-btn', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.location-option-btn').removeClass('selected active ios-active');
    ...
    setTimeout(() => {
        if (iOSSafariFix.isIOSSafari) {
            document.querySelectorAll('.service-button, .floor-select-btn, .location-select-btn, .quick-reply-btn, .floor-option-btn, .location-option-btn').forEach(btn => {
                iOSSafariFix.protectButton(btn);
            });
        }
    }, 1000);
...
$(document).ready(function () {
    ...
    // 初始化按鈕狀態
    $('.floor-option-btn, .location-option-btn').removeClass('selected active ios-active');
...
function selectService(serviceType) {
    ...
    // 清除之前的選擇
    $('.service-button').removeClass('selected active ios-active');
    ...
    if (currentService === 'office') {
        ...
        const formHTML = `
            ...
            <div class="form-group">
                ...
                <button class="location-select-btn location-select-btn-office" data-ios-protected="true">
            ...
            <div class="quick-replies">
                <button class="quick-reply-btn confirm-form-btn-office" data-ios-protected="true">
                    確認
                </button>
            </div>
        `;
        ...
        if (iOSSafariFix.isIOSSafari) {
            setTimeout(() => {
                document.querySelectorAll('.location-select-btn-office, .confirm-form-btn-office').forEach(btn => {
                    iOSSafariFix.protectButton(btn);
                });
            }, 100);
        }
        ...
        const formHTML = `
            ...
            <div class="form-group">
                <label class="form-label">樓層 <span>*</span></label>
                <button class="floor-select-btn floor-select-btn-dynamic" data-ios-protected="true">
            ...
            <div class="form-group">
                <label class="form-label">地點 <span>*</span></label>
                <button class="location-select-btn location-select-btn-dynamic" data-ios-protected="true">
                    <div class="location-display">
                        <span class="location-value">${locationDisplayText}</span>
                        <span class="location-arrow">▼</span>
                    </div>
            ...
            <div class="quick-replies">
                <button class="quick-reply-btn confirm-form-btn-dynamic" data-ios-protected="true">
                    確認
                </button>
            </div>
        `;
        ...
        if (iOSSafariFix.isIOSSafari) {
            setTimeout(() => {
                document.querySelectorAll('.floor-select-btn-dynamic, .location-select-btn-dynamic, .confirm-form-btn-dynamic').forEach(btn => {
                    iOSSafariFix.protectButton(btn);
                });
            }, 100);
        }
...
function bindFormEvents() {
    ...
    if (currentService === 'office') {
        ...
        $(document).on('click', '.location-select-btn-office', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).on('click', '.confirm-form-btn-office', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).on('click', '.floor-select-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).on('click', '.location-select-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).on('click', '.confirm-form-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
...
function askForPhoto() {
    ...
    if (hasPhoto) {
        ...
        photoHTML = `
            ...
            <div class="quick-replies" style="margin-top: 18px;">
                <button class="quick-reply-btn skip-photo-btn-dynamic" data-ios-protected="true">
                    移除照片
                </button>
                <button class="quick-reply-btn confirm-photo-btn-dynamic" data-ios-protected="true">
                    確認上傳
                </button>
            </div>
        `;
    ...
    setTimeout(() => {
        ...
        $(document).off('click', '.skip-photo-btn-dynamic').on('click', '.skip-photo-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).off('click', '.confirm-photo-btn-dynamic').on('click', '.confirm-photo-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        if (iOSSafariFix.isIOSSafari) {
            setTimeout(() => {
                document.querySelectorAll('.skip-photo-btn-dynamic, .confirm-photo-btn-dynamic').forEach(btn => {
                    iOSSafariFix.protectButton(btn);
                });
            }, 100);
        }
...
function showConfirmation() {
    ...
    const confirmationHTML = `
        ...
        <div class="quick-replies">
            <button class="quick-reply-btn edit-info-btn" data-ios-protected="true">
                ✏️ 修改資訊
            </button>
            <button class="quick-reply-btn submit-report-btn" data-ios-protected="true" style="background: #4a90e2 !important; color: white !important;">
                ✓ 確認提交
            </button>
        </div>
    `;
    ...
    setTimeout(() => {
        ...
        if (iOSSafariFix.isIOSSafari) {
            setTimeout(() => {
                document.querySelectorAll('.edit-info-btn, .submit-report-btn').forEach(btn => {
                    iOSSafariFix.protectButton(btn);
                });
            }, 100);
        }
...
function showSuccessPage() {
    ...
    const successHTML = `
        <div class="success-container">
            ...
            <div class="quick-replies">
                <button class="quick-reply-btn new-report-btn-dynamic" data-ios-protected="true">
                    新的通報
                </button>
            </div>
        </div>
    ...
    $(document).off('click', '.new-report-btn-dynamic').on('click', '.new-report-btn-dynamic', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    if (iOSSafariFix.isIOSSafari) {
        setTimeout(() => {
            document.querySelectorAll('.new-report-btn-dynamic').forEach(btn => {
                iOSSafariFix.protectButton(btn);
            });
        }, 100);
    }
...
function showFloorModal() {
    ...
    $(document).off('click', '.floor-option-btn-dynamic').on('click', '.floor-option-btn-dynamic', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.floor-option-btn-dynamic').removeClass('selected active ios-active');
    ...
    if (iOSSafariFix.isIOSSafari) {
        setTimeout(() => {
            iOSSafariFix.protectModal($('#floorModal')[0]);
            // 保護模態框內的按鈕
            document.querySelectorAll('#floorOptions .floor-option-btn-dynamic').forEach(btn => {
                iOSSafariFix.protectButton(btn);
            });
        }, 50);
    }
...
function showLocationModal() {
    ...
    const customLocationHTML = `
        <div class="custom-location-section">
            ...
            <button class="custom-location-confirm-btn custom-location-confirm-btn-dynamic" data-ios-protected="true">
                確認自訂地點
            </button>
        </div>
    `;
    ...
    $(document).off('click', '.location-option-btn-dynamic').on('click', '.location-option-btn-dynamic', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.location-option-btn-dynamic').removeClass('selected active ios-active');
    ...
    $(document).off('click', '.custom-location-confirm-btn-dynamic').on('click', '.custom-location-confirm-btn-dynamic', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    if (iOSSafariFix.isIOSSafari) {
        setTimeout(() => {
            iOSSafariFix.protectModal($('#locationModal')[0]);
            // 保護模態框內的按鈕
            document.querySelectorAll('#locationOptions .location-option-btn-dynamic, #locationOptions .custom-location-confirm-btn-dynamic').forEach(btn => {
                iOSSafariFix.protectButton(btn);
            });
        }, 50);
    }
