...
class IOSStateManager {
    ...
    // 清理所有可能的殘留狀態
    clearActiveStates() {
        if (!this.isIOS || this.stateLock) return;
        ...
        try {
            ...
            // 2. 清理所有按鈕的 active 類
            document.querySelectorAll('.floor-option-btn.active, .location-option-btn.active').forEach(btn => {
                btn.classList.remove('active');
            });

            // 3. 移除所有 :active 偽類狀態（通過添加和移除類來觸發重繪）
            document.body.classList.add('ios-state-reset');
            setTimeout(() => {
                document.body.classList.remove('ios-state-reset');
            }, 10);
    ...
    afterCloseModal() {
        ...
        // 確保焦點正確恢復
        setTimeout(() => {
            if (this.activeElement && this.activeElement.focus && !this.activeElement.disabled) {
                try {
                    this.activeElement.focus();
                } catch (e) {
                    console.log('無法恢復焦點:', e);
                }
            }
        }, 100);
    }

    // 添加視覺斷點（打破 Safari 的空間記憶）
    addVisualBreak() {
        if (!this.isIOS) return;

        const breakDiv = document.createElement('div');
        breakDiv.className = 'ios-visual-break';
    ...
    // 安全的按鈕點擊處理器
    safeButtonClick(button, callback) {
        if (!this.isIOS) {
            callback();
            return;
        }
        ...
        // 防止快速雙擊
        const now = Date.now();
        if (now - this.touchStartTime < 500) {
            return;
        }
...
// 初始化 iOS 狀態管理器
const iosManager = new IOSStateManager();
...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function(e) {
        ...
        // iOS 安全處理
        iosManager.safeButtonClick(this, () => {
            // 1. 清除所有同類按鈕的狀態
