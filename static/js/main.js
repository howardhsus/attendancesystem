// ==================== IOSSpatialBugShield - iOS Safari 狀態殘留修復 ====================
/* ============================================================================
 * IOSSpatialBugShield (Slim & Stable Edition)
 * 專門修復 iOS Safari 的 spatial active state leakage bug
 * ============================================================================ */
window.IOSSpatialBugShield = (function () {
    ...
    const isIOSSafari = (() => {
        ...
        const isSafari = /^((?!chrome|android).)*safari/i.test(ua) && /AppleWebKit/.test(ua);
        const isWebKit = /WebKit/.test(ua);
        const isOther = /CriOS|FxiOS|EdgiOS/.test(ua);

        return isIOS && (isSafari || isWebKit) && !isOther;
    })();

    console.log('IOSSpatialBugShield: iOS Safari detected?', isIOSSafari);

    /**
     * 切斷 Safari 的空間狀態記憶
     */
    function breakSpatialState() {
        if (!isIOSSafari) {
            console.log('IOSSpatialBugShield: Not iOS Safari, skipping');
            return;
        }

        console.log('IOSSpatialBugShield: Breaking spatial state...');
        ...
        document.body.appendChild(breakNode);

        // 4️⃣ 強制重繪 (reflow)
        void breakNode.offsetHeight;
        ...
        requestAnimationFrame(() => {
            ...
            console.log('IOSSpatialBugShield: Spatial state broken');
        });
    }
     ...
     * 安全打開 Modal
     */
    function openModalSafely(modalOpenFunction) {
        if (!isIOSSafari) {
            modalOpenFunction();
            return;
        }

        console.log('IOSSpatialBugShield: Opening modal safely...');
        ...
        requestAnimationFrame(() => {
            ...
            // 步驟 5: 確保 modal 內的按鈕沒有焦點
            setTimeout(() => {
                const modalButtons = document.querySelectorAll('.modal button, .modal [role="button"]');
     ...
     * 安全關閉 Modal
     */
    function closeModalSafely(modalCloseFunction) {
        if (!isIOSSafari) {
            modalCloseFunction();
            return;
        }

        console.log('IOSSpatialBugShield: Closing modal safely...');
...
const ButtonStateManager = {
    ...
    initButtonGroup(groupId, selector, singleSelect = true) {
        ...
        $(document).off('click', selector).on('click', selector, (e) => {
            if (window.IOSSpatialBugShield.isIOSSafari) {
...
$(document).ready(function () {
    // 全局監聽所有按鈕點擊，立即移除焦點
    $(document).on('click touchstart', 'button, [role="button"]', function (e) {
        if (window.IOSSpatialBugShield.isIOSSafari) {
            // 立即移除焦點
            $(this).blur();
...
function showFloorModal() {
    // 使用安全方式打開 Modal
    window.IOSSpatialBugShield.openModalSafely(() => {
        ...
        // 綁定事件前先移除舊的
        $(document).off('click', '.floor-option-btn-dynamic');

        $(document).on('click', '.floor-option-btn-dynamic', function () {
            if (window.IOSSpatialBugShield.isIOSSafari) {
                // iOS Safari 特殊處理
                $(this).blur();
            }
...
function showLocationModal() {
    ...
    window.IOSSpatialBugShield.openModalSafely(() => {
        ...
        // 移除舊的事件綁定
        $(document).off('click', '.location-option-btn-dynamic');
        $(document).off('click', '.custom-location-confirm-btn-dynamic');

        // 綁定新的事件
        $(document).on('click', '.location-option-btn-dynamic', function () {
            if (window.IOSSpatialBugShield.isIOSSafari) {
                $(this).blur();
            }
        ...
        $(document).on('click', '.custom-location-confirm-btn-dynamic', function () {
            if (window.IOSSpatialBugShield.isIOSSafari) {
                $(this).blur();
            }
...
function closeFloorModal() {
    window.IOSSpatialBugShield.closeModalSafely(() => {
        $('#floorModal').hide();
    });
}
