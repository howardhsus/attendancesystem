...
const iOSStateFixer = {
    ...
    forceRepaint: function (element) {
        if (!element || !this.isIOS()) return;

        // 方法1: 觸發 reflow
        const display = element.style.display;
        element.style.display = 'none';
        void element.offsetHeight; // 觸發 reflow
        element.style.display = display;

        // 方法2: 觸發 GPU 加速重繪
        element.style.transform = 'translateZ(0)';
        void element.offsetWidth; // 再次觸發 reflow
    ...
    clearButtonStates: function () {
        ...
        // 移除所有可能的活動狀態
        $('.floor-option-btn, .location-option-btn, .service-button, .quick-reply-btn')
            .removeClass('active selected')
            .blur();

        // 強制觸發 body 重繪
        this.forceRepaint(document.body);

        // 額外的修復：短暫禁用 pointer-events
    ...
    // 安全的點擊處理器
    safeClickHandler: function (callback) {
        return function (e) {
            if (iOSStateFixer.isIOS()) {
                e.preventDefault();
                e.stopPropagation();

                // 立即清除所有狀態
                iOSStateFixer.clearButtonStates();
    ...
    // 修復模態框顯示
    safeShowModal: function (modalId, callback) {
        if (this.isIOS()) {
            // 先清除所有狀態
            ...
            setTimeout(() => {
                ...
                // 延遲啟用模態框內的點擊
                const $modalContent = $(modalId + ' .modal-body');
                $modalContent.css('pointer-events', 'none');
                setTimeout(() => {
...
const ButtonStateManager = {
    ...
    initButtonGroup(groupId, selector, singleSelect = true) {
        ...
        // 綁定事件 - 使用 iOS 修復版本
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', selector).on('touchend', selector, (e) => {
                e.preventDefault();
                e.stopPropagation();
                this.handleButtonClickIOS(groupId, $(e.currentTarget));
                return false;
            });
    ...
    // iOS 專用處理
    handleButtonClickIOS(groupId, $button) {
        const group = this.buttonGroups[groupId];
        if (!group) return;
...
function setupStableButtonHandlers() {
    // iOS 修復：使用 touchend 事件
    if (iOSStateFixer.isIOS()) {
        // 樓層按鈕 - iOS 專用處理
        $(document).off('touchend', '.floor-option-btn').on('touchend', '.floor-option-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        // 地點按鈕 - iOS 專用處理
        $(document).off('touchend', '.location-option-btn').on('touchend', '.location-option-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        // 服務按鈕 - iOS 專用處理
        $(document).off('touchend', '.service-button').on('touchend', '.service-button', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        // 快速回復按鈕 - iOS 專用處理
        $(document).off('touchend', '.quick-reply-btn').on('touchend', '.quick-reply-btn', function (e) {
            e.preventDefault();
            e.stopPropagation();
    ...
    // 防止快速雙擊
    let lastClickTime = 0;
    $(document).on('click touchend', '.floor-option-btn, .location-option-btn, .service-button, .quick-reply-btn', function (e) {
        const now = Date.now();
...
function bindEvents() {
    ...
    $('#closeImageModalBtn').off('click').on('click', function () {
        closeImageModal();
    });

    $('#closeFloorModalBtn').off('click').on('click', function () {
        closeFloorModal();
    });

    $('#closeLocationModalBtn').off('click').on('click', function () {
        closeLocationModal();
    });
...
$(document).ready(function () {
    ...
    // iOS 修復：在初始化時添加 CSS 類
    if (iOSStateFixer.isIOS()) {
        $('body').addClass('ios-device');
        console.log('iOS 設備檢測：已啟用 iOS 專用修復');
    }
...
function bindFormEvents() {
    ...
    if (currentService === 'office') {
        ...
        // iOS 修復：地點選擇按鈕
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '.location-select-btn-office').on('touchend', '.location-select-btn-office', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
            ...
            $(document).off('touchend', '.confirm-form-btn-office').on('touchend', '.confirm-form-btn-office', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
        ...
        // iOS 修復：樓層選擇按鈕
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '.floor-select-btn-dynamic').on('touchend', '.floor-select-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
            ...
            $(document).off('touchend', '.location-select-btn-dynamic').on('touchend', '.location-select-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
            ...
            $(document).off('touchend', '.confirm-form-btn-dynamic').on('touchend', '.confirm-form-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
...
function askForPhoto() {
    ...
    setTimeout(() => {
        // iOS 修復：上傳區域點擊
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '.upload-area-dynamic').on('touchend', '.upload-area-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
            ...
            $(document).off('touchend', '.skip-photo-btn-dynamic').on('touchend', '.skip-photo-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
            ...
            $(document).off('touchend', '.confirm-photo-btn-dynamic').on('touchend', '.confirm-photo-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
...
function showConfirmation() {
    ...
    setTimeout(() => {
        // iOS 修復：修改資訊按鈕
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '.edit-info-btn').on('touchend', '.edit-info-btn', function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            $(document).off('touchend', '.submit-report-btn').on('touchend', '.submit-report-btn', function (e) {
                e.preventDefault();
                e.stopPropagation();
                iOSStateFixer.clearButtonStates();
                setTimeout(() => {
...
function showSuccessPage() {
    ...
    // iOS 修復：新的通報按鈕
    if (iOSStateFixer.isIOS()) {
        $(document).off('touchend', '.new-report-btn-dynamic').on('touchend', '.new-report-btn-dynamic', function (e) {
            e.preventDefault();
            e.stopPropagation();
            iOSStateFixer.clearButtonStates();
            setTimeout(() => {
...
function showFloorModal() {
    iOSStateFixer.safeShowModal('#floorModal', function () {
        ...
        // iOS 修復：模態框內按鈕事件
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '#floorModal .floor-option-btn-dynamic').on('touchend', '#floorModal .floor-option-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                // 設置選擇狀態
                $('#floorModal .floor-option-btn-dynamic').removeClass('selected');
                $btn.addClass('selected');
            ...
            $(document).off('click', '#floorModal .floor-option-btn-dynamic').on('click', '#floorModal .floor-option-btn-dynamic', function () {
...
function showLocationModal() {
    iOSStateFixer.safeShowModal('#locationModal', function () {
        let locations = [];
        let modalTitle = '';
        ...
        // iOS 修復：模態框內按鈕事件
        if (iOSStateFixer.isIOS()) {
            $(document).off('touchend', '#locationModal .location-option-btn-dynamic').on('touchend', '#locationModal .location-option-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                // 設置選擇狀態
                $('#locationModal .location-option-btn-dynamic').removeClass('selected');
                $btn.addClass('selected');

                // 清除自訂地點輸入
                $('#modalCustomLocationInput').val('');
            ...
            $(document).off('touchend', '#locationModal .custom-location-confirm-btn-dynamic').on('touchend', '#locationModal .custom-location-confirm-btn-dynamic', function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            // 自訂地點輸入框事件
            $(document).off('focus', '#modalCustomLocationInput').on('focus', '#modalCustomLocationInput', function () {
            ...
            $(document).off('blur', '#modalCustomLocationInput').on('blur', '#modalCustomLocationInput', function () {
                if (!$(this).val().trim()) {
            ...
            // 輸入時清除預選地點
            $(document).off('input', '#modalCustomLocationInput').on('input', '#modalCustomLocationInput', function () {
                $('#locationModal .location-option-btn-dynamic').removeClass('selected');
                iOSStateFixer.clearButtonStates();
            });
        } else {
            $(document).off('click', '#locationModal .location-option-btn-dynamic').on('click', '#locationModal .location-option-btn-dynamic', function () {
                const locationId = $(this).data('location');
                selectLocation(locationId);
            });

            $(document).off('click', '#locationModal .custom-location-confirm-btn-dynamic').on('click', '#locationModal .custom-location-confirm-btn-dynamic', confirmCustomLocation);

            $(document).off('focus blur', '#modalCustomLocationInput')
                .on('focus', '#modalCustomLocationInput', function () {
                    if ($(this).hasClass('placeholder-active')) {
                        $(this).removeClass('placeholder-active');
                        $(this).val('');
                    }
                })
                .on('blur', '#modalCustomLocationInput', function () {
                    if (!$(this).val().trim()) {
                        $(this).addClass('placeholder-active');
                        $(this).val('輸入自訂地點');
                    }
...
function closeFloorModal() {
    iOSStateFixer.safeCloseModal($('#floorModal'));
}

function closeLocationModal() {
    iOSStateFixer.safeCloseModal($('#locationModal'));
}

function closeImageModal() {
    iOSStateFixer.safeCloseModal($('#imageModal'));
}

// 新增：統一的模態框關閉函數
function closeModal($modal) {
    iOSStateFixer.safeCloseModal($modal);
}
