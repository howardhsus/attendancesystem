...
const ButtonStateManager = {
    ...
    handleButtonClick(groupId, $button) {
        ...
        if (group.singleSelect) {
            ...
            group.selected = wasSelected ? null : buttonId;
        }
        ...
        // 修復問題3：確保只有選中的按鈕有 selected 類
        $(group.selector).not($button).removeClass('selected');
    },

    // 程式化選擇按鈕 - 修復問題3：清除其他按鈕狀態
...
function setupStableButtonHandlers() {
    ...
    $(document).off('click', '.floor-option-btn').on('click', '.floor-option-btn', function (e) {
        ...
        // 修復問題3：只清除同類按鈕的狀態
        $btn.closest('.modal-body').find('.floor-option-btn').removeClass('selected');
    ...
    $(document).off('click', '.location-option-btn').on('click', '.location-option-btn', function (e) {
        ...
        // 修復問題3：只清除同類按鈕的狀態
        $btn.closest('.modal-body').find('.location-option-btn').removeClass('selected');
...
function skipPhoto() {
    ...
    // 修復問題2：確保跳過按鈕的選取狀態正確
    const $skipBtn = $('.skip-photo-btn-dynamic');
