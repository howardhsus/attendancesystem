...
// ==================== 動態狀態與資料 (樂觀渲染骨架) ====================
let APP_CONFIG = {
    'office': { name_tw: '辦公大樓', floors: '1-88', locations_tw: [] },
    'shopping': { name_tw: '購物中心', floors: 'B1,1,2,3,4,5', locations_tw: [] },
    'observatory': { name_tw: '觀景台', floors: '89,91,101', locations_tw: [] }
};
...
function askForPhoto() {
    ...
    // 取消 display: none 隱藏機制，讓按鈕保留在畫面上供自由點擊
...
function skipPhoto() {
    ...
    if (uploadedFilePreview) {
        ...
        // 瞬間刪除舊的氣泡，避免畫面留存兩個一樣的氣泡
        DOM.selectAll('.user-message').forEach(m => { if (m.textContent.includes('不上傳照片') || m.textContent.includes('已上傳照片')) m.remove(); });
...
function confirmPhoto() {
    ...
    // 瞬間刪除舊的氣泡
    DOM.selectAll('.user-message').forEach(m => { if (m.textContent.includes('已上傳照片') || m.textContent.includes('不上傳照片')) m.remove(); });
...
async function showFloorModal() {
    ...
    if (!isConfigLoaded) return showToast("資料載入失敗，請重試", "warning");
