/* ============================================================================
 * Safari Visual State Sanitizer
 * 解決 iOS Safari 視覺狀態記憶問題（第 2 層防護）
 * ========================================================================== */
function sanitizeSafariVisualState() {
    ...
    document.body.classList.add('safari-visual-sanitize');

    requestAnimationFrame(() => {
        document.body.classList.remove('safari-visual-sanitize');
    });
}

// 在關鍵場景切換後呼叫此函數
function applySafariVisualFix() {
    sanitizeSafariVisualState();
}

/* ============================================================================
 * IOSSpatialBugShield
 * 專治 iOS Safari / WebView 的 Modal 位置記憶 Active Bug
 * ========================================================================== */
window.IOSSpatialBugShield = (function () {

    // 判斷是不是 iOS Safari（含 WebView）
    const isIOSSafari = (() => {
        const ua = navigator.userAgent;
    ...
    // 修復模態框內按鈕的點擊事件
    function fixModalButtonEvents(modalSelector) {
        if (!isIOSSafari) return;
    ...
    if (currentService === 'office') {
        ...
        const formHTML = `
            ...
            <div class="form-group">
                ...
                <button class="location-select-btn location-select-btn-office" data-ios-modal-trigger="location">
        ...
        const formHTML = `
            ...
            <div class="form-group">
                <label class="form-label">樓層 <span>*</span></label>
                <button class="floor-select-btn floor-select-btn-dynamic" data-ios-modal-trigger="floor">
            ...
            <div class="form-group">
                <label class="form-label">地點 <span>*</span></label>
                <button class="location-select-btn location-select-btn-dynamic" data-ios-modal-trigger="location">
                    <div class="location-display">
                        <span class="location-value">${locationDisplayText}</span>
                        <span class="location-arrow">▼</span>
                    </div>
...
function bindFormEvents() {
    ...
    if (currentService === 'office') {
        ...
        // 使用 IOSSpatialBugShield 安全地開啟模態框
...
function showFloorModal() {
    ...
    $(document).off('click', '.floor-option-btn-dynamic').on('click', '.floor-option-btn-dynamic', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.floor-option-btn-dynamic').removeClass('selected active');
...
function showLocationModal() {
  ...
  `);
    ...
    $(document).off('click', '.location-option-btn-dynamic').on('click', '.location-option-btn-dynamic', function (e) {
        ...
        // 1. 清除所有同類按鈕的狀態
        $('.location-option-btn-dynamic').removeClass('selected active');
