...
// ==================== 動態狀態與資料 (樂觀渲染骨架) ====================
// 預載基礎骨架，讓畫面能 0 秒瞬間渲染，稍後會被背景 API 覆蓋為最新資料
let APP_CONFIG = {
    ...
    configFetchPromise = fetch(API_CONFIG.MAIN_API + "?action=getConfig")
        ...
        .then(data => {
            if (data.success && data.config) {
                APP_CONFIG = data.config; // 無縫覆蓋為最新資料
                isConfigLoaded = true;
            } else {
...
function bindGlobalEvents() {
    ...
    DOM.delegate('click', '.new-report-btn-dynamic', () => {
        window.location.href = window.location.pathname; // 重置時移除參數回首頁
    });
}
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (autoDetectedLocation) {
                selectedLocation = autoDetectedLocation; // 樂觀信任網址參數
                customLocation = '';
            }
            setStep('input_form');
        }, 800);
...
function previewUploadedImage() {
    if (uploadedFilePreview) {
        ...
        showModalWithDelay(DOM.select('#imageModal'));
    }
}
...
// ==================== 動態模態框控制 (防護網機制) ====================
function showModalWithDelay(modalElement) {
    if (document.activeElement) document.activeElement.blur();
    document.body.style.overflow = 'hidden';
    modalElement.style.pointerEvents = 'none'; // 防穿透護盾開啟
    modalElement.style.display = 'flex';
    setTimeout(() => modalElement.style.pointerEvents = 'auto', 350); // 0.35秒後解除護盾
}

async function showFloorModal() {
    // 防護網：極限手速或網路過慢時，強制等待最新資料
    if (!isConfigLoaded) {
        showToast("正在同步最新樓層資料...", "info");
    ...
    showModalWithDelay(DOM.select('#floorModal'));
}

async function showLocationModal() {
    // 防護網：極限手速或網路過慢時，強制等待最新資料
    if (!isConfigLoaded) {
        showToast("正在同步最新地點資料...", "info");
        await configFetchPromise;
    }
    if (!isConfigLoaded) return showToast("資料載入失敗，請重試", "warning");
    ...
    showModalWithDelay(DOM.select('#locationModal'));
}
