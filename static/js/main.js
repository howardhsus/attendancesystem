...
function fixButtonTouchStates() {
    const platform = getPlatformInfo();

    if (platform.isIOS) {
        console.log('應用按鈕觸控狀態修復');

        // 延遲執行確保 DOM 完全加載
        setTimeout(() => {
            // 修復所有快速回覆按鈕
            $('.quick-reply-btn, .skip-photo-btn-dynamic, .confirm-photo-btn-dynamic, .edit-info-btn, .submit-report-btn, .new-report-btn-dynamic').each(function () {
                const $btn = $(this);

                // 添加 iOS 修復類別
                $btn.addClass('ios-btn-fix');

                // 強制設置初始樣式
                $btn.css({
                    '-webkit-tap-highlight-color': 'rgba(74, 144, 226, 0.05)',
                ...
                // 修復 touchstart 事件
                $btn.off('touchstart').on('touchstart', function (e) {
                    e.preventDefault();
                    e.stopPropagation();

                    // 添加臨時狀態
                    $(this).addClass('ios-touch-active');
                ...
                // 修復 touchend 事件
                $btn.off('touchend touchcancel').on('touchend touchcancel', function (e) {
                    e.preventDefault();
                    e.stopPropagation();

                    // 移除臨時狀態
                    $(this).removeClass('ios-touch-active');
            ...
            if (platform.version && platform.version.major === 16 && platform.version.minor === 2) {
                console.log('應用 iOS 16.2 特別按鈕修復');

                const style = document.createElement('style');
                style.id = 'ios-16-2-button-fix';
                style.textContent = `
                    /* iOS 16.2 特別修復 */
                    .ios-btn-fix {
                ...
                // 移除舊樣式
                const oldStyle = document.getElementById('ios-16-2-button-fix');
