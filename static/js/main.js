/* ==================== iOS Safari Active Ghosting 修復（全站級） ==================== */
(function () {
    let activeEl = null;
    ...
    document.addEventListener('touchstart', e => {
        const btn = e.target.closest(
            'button, .floor-option-btn, .location-option-btn, .quick-reply-btn, .service-button'
        );
        if (btn) press(btn);
    }, { passive: true });

    document.addEventListener('touchend', clearActive);
    document.addEventListener('touchcancel', clearActive);
    ...
    // Safari 頁面導航時清除所有狀態
    window.addEventListener('pageshow', clearActive);
})();
...
const ButtonStateManager = {
    ...
    handleButtonClick(groupId, $button) {
        ...
        if (group.singleSelect) {
            ...
            // 使用 requestAnimationFrame 確保狀態同步
