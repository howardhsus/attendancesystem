...
function applyQuickButtonFixes() {
    const platform = getPlatformInfo();

    if (platform.isIOS) {
        console.log('應用快速按鈕 iOS 觸控修復');

        // 延遲綁定以確保 DOM 已加載
        setTimeout(() => {
            // 修復快速操作按鈕的觸控問題
            const quickButtons = [
                '.skip-photo-btn-dynamic',
                '.edit-info-btn',
                '.new-report-btn-dynamic'
            ];

            quickButtons.forEach(selector => {
                $(document).off('touchstart', selector);
                $(document).off('touchend', selector);
                $(document).off('touchcancel', selector);

                // 使用更積極的事件處理
                $(document).on('touchstart', selector, function (e) {
                    console.log('快速按鈕 touchstart:', selector);
                ...
                $(document).on('touchend touchcancel', selector, function (e) {
                    console.log('快速按鈕 touchend:', selector);

                    const $btn = $(this);

                    // 移除臨時類別
                    $btn.removeClass('ios-tap-active');
            ...
            // 針對 iOS 16.2 的額外修復
            if (platform.version && platform.version.major === 16 && platform.version.minor === 2) {
                console.log('應用 iOS 16.2 快速按鈕特別修復');

                const style = document.createElement('style');
                style.id = 'ios-quick-btn-fixes';
                style.textContent = `
                    /* 極端修復：完全禁用偽類 */
                    .skip-photo-btn-dynamic:active,
                    .edit-info-btn:active,
                    .new-report-btn-dynamic:active {
                        opacity: 1 !important;
                        background-color: white !important;
                    }
                    
                    /* 使用 JavaScript 控制的狀態類別 */
                    .skip-photo-btn-dynamic.ios-tap-active,
                    .edit-info-btn.ios-tap-active,
                    .new-report-btn-dynamic.ios-tap-active {
                ...
                // 移除舊樣式如果存在
                const oldStyle = document.getElementById('ios-quick-btn-fixes');
...
function applyCompatibilityFixes() {
    ...
    // 平台特定修復
    if (platform.isIOS) {
        applyIOSFixes(platform);
        // 新增：快速按鈕觸控修復
        applyQuickButtonFixes();
...
function applyUniversalFixes() {
    ...
    setTimeout(() => {
        $('.floor-select-btn, .location-select-btn').each(function () {
            ...
            // 移除所有衝突的類別
            $btn.removeClass('selected active ios-tap-fix');
    ...
    // 2. 統一事件處理
    $(document).off('click touchstart', '.floor-select-btn, .location-select-btn')
        .on('click touchstart', '.floor-select-btn, .location-select-btn', function (e) {
            // 防止默認行為但允許事件傳播
...
// ==================== iOS 專用修復 ====================
function applyIOSFixes(platform) {
    console.log('應用 iOS 修復，版本:', platform.version);
    ...
    setTimeout(() => {
        // 使用更激進的樣式重置
        const style = document.createElement('style');
        style.id = 'ios-fix-styles';
        style.textContent = `
            /* 完全禁用 iOS 的偽類狀態 */
            button[data-ios-fix].floor-select-btn,
            button[data-ios-fix].location-select-btn {
            ...
            /* 防止 :active 狀態 */
            button[data-ios-fix].floor-select-btn:active,
            button[data-ios-fix].location-select-btn:active {
                background-color: white !important;
                opacity: 1 !important;
            ...
            /* 防止 :focus 狀態 */
            button[data-ios-fix].floor-select-btn:focus,
            button[data-ios-fix].location-select-btn:focus {
        ...
        // 移除舊的樣式如果存在
        const oldStyle = document.getElementById('ios-fix-styles');
        if (oldStyle) oldStyle.remove();

        document.head.appendChild(style);

        // 動態添加 data-ios-fix 屬性
        $('.floor-select-btn, .location-select-btn').attr('data-ios-fix', 'true');

    }, 200);
}

// ==================== iOS 16.2 特別修復 ====================
function applyIOS16_2Fixes() {
    console.log('應用 iOS 16.2 特別修復');

    // 創建專門針對 iOS 16.2 的樣式
    const style = document.createElement('style');
    style.id = 'ios-16-2-fixes';
    style.textContent = `
        /* 極端修復：完全禁用所有觸控狀態 */
        .floor-select-btn,
        .location-select-btn {
...
function applyAndroidFixes() {
    console.log('應用 Android 修復');

    // Android Chrome 通常不需要特別修復
    // 但可以添加一些優化
    setTimeout(() => {
        $('.floor-select-btn, .location-select-btn').css({
...
$(document).ready(function () {
    ...
    setTimeout(() => {
        ...
        $('.floor-select-btn, .location-select-btn').each(function () {
            const $btn = $(this);
            $btn.removeClass('selected active ios-tap-fix tap-active');
            $btn.css({
                'background-color': 'white',
                'border-color': '#ddd'
            });
...
function showReportForm() {
    ...
    if (currentService === 'office') {
        ...
        const locationBtnClass = (selectedLocation || customLocation) ?
            'location-select-btn location-select-btn-office has-selection ios-fixed' :
            'location-select-btn location-select-btn-office ios-fixed';
        ...
        const formHTML = `
            ...
            <div class="form-group">
                ...
                <button class="${locationBtnClass}" data-ios-fix="true">
        ...
        const floorBtnClass = selectedFloor ?
            'floor-select-btn floor-select-btn-dynamic has-selection ios-fixed' :
            'floor-select-btn floor-select-btn-dynamic ios-fixed';

        const locationBtnClass = (selectedLocation || customLocation) ?
            'location-select-btn location-select-btn-dynamic has-selection ios-fixed' :
            'location-select-btn location-select-btn-dynamic ios-fixed';
        ...
        const formHTML = `
            ...
            <div class="form-group">
                <label class="form-label">樓層 <span>*</span></label>
                <button class="${floorBtnClass}" data-ios-fix="true">
        ...
        setTimeout(() => {
            ...
            if (platform.isIOS) {
                ...
                [$floorBtn, $locationBtn].forEach($btn => {
                    if ($btn.length) {
                        $btn.css({
                            'background-color': 'white',
                            'border-color': '#ddd',
...
function showFloorModal() {
    ...
    // 調試：確認 selectedFloor 的值
    console.log('可用樓層:', floors);
    console.log('當前 selectedFloor:', selectedFloor);
    ...
    const floorOptionsHTML = floors.map(floor => {
        ...
        console.log(`樓層 ${floor}: isSelected = ${isSelected}`);
...
function selectLocation(locationId) {
    ...
    if (platform.isIOS) {
        ...
        if (currentService === 'office') {
            const $locationBtn = $('.location-select-btn-office');
            const $value = $locationBtn.find('.location-value');

            // 直接設置文本
            $value.text(locationLabel);
            $value.css('color', '#2d3436');
