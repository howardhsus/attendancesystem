...
const iOSFixManager = {
    ...
    // 修復按鈕狀態洩漏的核心函數
    fixButtonStateLeak: function (button) {
        if (!this.isIOS() || !button) return;
        ...
        // 3. 短暫延遲後強制重繪
        setTimeout(() => {
            // 觸發 reflow 來打破 iOS 的位置記憶
            void button.offsetHeight;
        }, 10);
    },

    // 修復模態框顯示
    fixModalShow: function (modalId) {
        if (!this.isIOS()) return;

        // 清除所有按鈕狀態
        $('.floor-option-btn, .location-option-btn').removeClass('active selected');
...
function bindFormEvents() {
    // 辦公大樓表單事件
    if (currentService === 'office') {
        $('.floor-input-office').off('focus blur input').on({
            focus: handleFloorFocus,
            blur: handleOfficeFloorBlur,
            input: handleOfficeFloorInput
        });

        $('.description-input-office').off('focus blur input').on({
            focus: handleDescriptionFocus,
            blur: handleDescriptionBlur,
            input: handleDescriptionInput
        });

        $('.location-select-btn-office').off('click').on('click', function () {
            showLocationModal();
        });

        $('.confirm-form-btn-office').off('click').on('click', confirmForm);
    } else {
        // 購物中心/觀景台表單事件
        $('.description-input-dynamic').off('focus blur input').on({
            focus: handleDescriptionFocus,
            blur: handleDescriptionBlur,
            input: handleDescriptionInput
        });

        $('.floor-select-btn-dynamic').off('click').on('click', function () {
            showFloorModal();
        });

        $('.location-select-btn-dynamic').off('click').on('click', function () {
            showLocationModal();
        });

        $('.confirm-form-btn-dynamic').off('click').on('click', confirmForm);
    }
}
...
function askForPhoto() {
    ...
    // 簡單的事件綁定
    setTimeout(() => {
        $('.upload-area-dynamic').off('click').on('click', handleUploadClick);
        $('.skip-photo-btn-dynamic').off('click').on('click', skipPhoto);
        $('.confirm-photo-btn-dynamic').off('click').on('click', confirmPhoto);
        $('.preview-image-dynamic').off('click').on('click', previewUploadedImage);
    }, 100);
}
...
function showConfirmation() {
    ...
    // 簡單的事件綁定
    setTimeout(() => {
        $('.edit-info-btn').off('click').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $('.submit-report-btn').off('click').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            submitReport();
        });
...
function showSuccessPage() {
    ...
    // 簡單的事件綁定
    setTimeout(() => {
        $('.new-report-btn-dynamic').off('click').on('click', function () {
...
function showFloorModal() {
    ...
    if (iOSFixManager.isIOS()) {
        iOSFixManager.fixModalShow('#floorModal');
    }
    ...
    // 簡單的事件綁定
    $('.floor-option-btn-dynamic').off('click').on('click', function () {
        const floor = $(this).data('floor');

        // iOS 修復：點擊時清除狀態
        if (iOSFixManager.isIOS()) {
            iOSFixManager.fixButtonStateLeak(this);
        }
...
function showLocationModal() {
    // iOS 修復：顯示模態框前先清除狀態
    if (iOSFixManager.isIOS()) {
        iOSFixManager.fixModalShow('#locationModal');
    }
    ...
    // 簡單的事件綁定
    $('.location-option-btn-dynamic').off('click').on('click', function () {
    ...
    $('.custom-location-confirm-btn-dynamic').off('click').on('click', function () {
        // iOS 修復：點擊時清除狀態
        if (iOSFixManager.isIOS()) {
            iOSFixManager.fixButtonStateLeak(this);
        }
    ...
    // 自訂地點輸入框事件
    $('#modalCustomLocationInput').off('focus blur input').on({
        focus: function () {
